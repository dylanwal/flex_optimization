...
def main():
    ...
    # STOP CRITERIA
    # method = fo.methods.MethodSobol(problem, fo.stop_criteria.StopComputationTime(0.05))
    # method = fo.methods.MethodSobol(problem, [fo.stop_criteria.StopComputationTime(0.11), fo.stop_criteria.StopFunctionEvaluation(100)])  # or
    # method = fo.methods.MethodSobol(problem, [[fo.stop_criteria.StopComputationTime(0.1), fo.stop_criteria.StopFunctionEvaluation(1000)]])  # and
    # method = fo.methods.MethodSobol(problem, fo.stop_criteria.StopRelativeChange(cut_off_value=1E-10, cut_off_steps=10))   # no improvement for 10 steps
    # method = fo.methods.MethodSobol(problem, fo.stop_criteria.StopRelativeChange(cut_off_value=0.4, cut_off_steps=1))  # stop imidately when there is no relative imporvement
    # method = fo.methods.MethodSobol(problem, fo.stop_criteria.StopAbsoluteChange(cut_off_value=0.05, cut_off_steps=10))
    # method = fo.methods.MethodSobol(problem, fo.stop_criteria.StopRate(cut_off_rate=0.05, prior_steps=5, cut_off_steps=2))
    ...
    vis = fo.VizOptimization(method.recorder, true_func=problem.evaluate)
    fig = vis.plot_3d_vis()
    fig.show()
